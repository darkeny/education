...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conn->prepare('SELECT nome, email, senha FROM User WHERE id = :id');
        $stmt->bindParam(':id', $id);
        $stmt->execute();

        $userArgs = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    <div class="container">
        <div class="container-login-small content-center w-100 ">
            <div class="shadow p-5 brd-15 animate__animated animate__pulse">
                ...
                <form action="../../../backend/src/updateUser.php" method="post" class="needs-validation" novalidate>
                    <input type="hidden" name="id" value="<?php echo $_GET['id']; ?>">
                    <div class="form-floating">
                        <input value="<?php echo $userArgs['nome']; ?>" type="text" id="name" name="nome" class="form-control hidden-focus" required placeholder="Insira seu nome">
                        <label for="name">Insira o seu nome</label>
                    </div>
                    <div class="form-floating my-2">
                        <input value="<?php echo $userArgs['email']; ?>" type="email" id="email" name="email" class="form-control hidden-focus" required placeholder="Insira seu email">
                        <label for="name">Insira o seu email</label>
                    </div>
                    <div class="form-floating">
                        <input value="<?php echo $userArgs['senha']; ?>" type="password" id="password" name="senha" class="form-control hidden-focus" required placeholder="Insira seu nome">
